...
Interestingly, for \( r \) slightly smaller than \( a \), the reference force, and its mesh-based PM approximation, still provides a good fit to the inverse-square behavior.
...
\end{figure}
\autoref{fig:reference-force-combined}\subref{fig:reference-force-approx-sub} indicates that using the \( S_2 \) shape function in the PM calculation results in a force that more closely approximates the inverse-square law at small \( r \), whereas the \( S_1 \)-based method begins to show noticeable deviations.
However, \autoref{fig:reference-force-combined}\subref{fig:reference-force-error-sub} shows that the \( S_2 \)-based approximation exhibits higher mean-normalized RMSD error compared to \( S_1 \), especially at intermediate distances.
